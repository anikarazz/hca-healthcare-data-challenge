...
function App() {
  ...
  return (
    <div>
      {/* Filters & Map Section */}
      <div style={{ display: "flex", gap: "20px", alignItems: "stretch", height: "auto" }}>
        {/* Left side - Filters */}
        <div
          style={{
            width: "50%",
        ...
        >
          ...
          <div style={{ display: "flex", gap: "20px", marginTop: "5px" }}>
            ...
            <button style={refreshButtonStyle} onClick={() => { setSupervisors([]); setSelectedFacility(null); }}>Refresh</button>
          </div>
        </div>
      ...
      <div style={{ marginTop: "-20px", padding: "13px", width: "50%" }}>
        <h3>Results:</h3>
        ...
        {supervisors.length === 0 ? (
          ...
          supervisors.map((sup) => (
            <div key={sup.Emp34Id} style={resultCardStyle}>
              <h3>{sup.EmpFirstName} {sup.EmpLastName}</h3>
              <p><strong>ID:</strong> {sup.Emp34Id}</p>
              <p><strong>Location:</strong> {sup.EmpLocationDesc}</p>
              <p><strong>Position:</strong> {sup.EmpPositionDesc}</p>
              <p><strong>Department:</strong> {sup.EmpDepartmentName}</p>
              <p>
              ...
              {showManager[sup.Emp34Id] && sup.Mgr34Id && (
                <div style={managerCardStyle}>
                  <h4>{sup.MgrName}</h4>
                  <p><strong>ID:</strong> {sup.Mgr34Id}</p>
                  <p><strong>Title:</strong> {sup.MgrTitle}</p>
                </div>
              )}
              <p><strong>Availability:</strong> {sup.Availability}</p>
              {["On Call", "Available"].includes(sup.Availability) && (
                <button style={requestButtonStyle} onClick={() => alert("Request sent!")}>Request</button>
              )}
            </div>
          ))
        )}
